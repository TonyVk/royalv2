$(function(){
	...
	$('.users').on('click', '.spec', function(){
		...
		let player = $('.spectate').attr('id');
			...
			$('.spectate').fadeOut();
			$.post('http://optimum_posmatraj/select', JSON.stringify({id: target}));
	...
	$('.users').on('click', '.kick', function(){
		...
		if (reason == null || reason == "") {
		    ...
		    $('.spectate').fadeOut();
			$.post('http://optimum_posmatraj/kick', JSON.stringify({id: target, reason: reason}));
		}
	});

	$('.header').on('click', '#close', function(){
		$('.spectate').fadeOut();
		$.post('http://optimum_posmatraj/quit');
	});
	...
	$(document).keyup(function(e){
		if (e.keyCode == 27){
			$('.spectate').fadeOut();
			$.post('http://optimum_posmatraj/close');
		}
	})
	...
	for (var i = 0; i < data.length; i++) {
		...
		let element = 	'<div class="user">' +
							...
							'<span class="user-actions">' +
								'<button class="spec" data-spectate="' + id + '">Posmatraj</button>' +
