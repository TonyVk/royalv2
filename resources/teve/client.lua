...
VolumeCheck = function(id)
    if data[id]['DUI'] then
        if data[id]['ActualVolume'] ~= data[id]['Volume'] then
			...
			if not jednom then
				SendDuiMouseMove(duiLong, 771, 392)
				Wait(250)
                                        ...
                                        CreateThread(function()
                                            if v['DUI'] then
												if not trovo then
													...
													SetDuiUrl(v['DUI']['Long'], Config['URL2']:format(v['URL'], (math.floor(GetGameTimer() / 1000) + v['Time']) - v['Started']))
												end
                                            end
                                        end)
                                    end
                ...
                if DoesEntityExist(obj) then
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= v['Distance'] then
                        if SFHandle ~= nil then
							local duiLong
							if not trovo then
								duiLong = CreateDui(Config['URL']:format(v['URL'], (math.floor(GetGameTimer() / 1000) + v['Time']) - v['Started']), 1280, 720)
							else
								duiLong = CreateDui(Config['URL2']:format(v['URL'], (math.floor(GetGameTimer() / 1000) + v['Time']) - v['Started']), 1280, 720)
							end
