		...
		if #xPlayers > 0 then
			for i=1, #xPlayers, 1 do
				...
				if xPlayer ~= -1 and xPlayer ~= nil then
					...
					--end)
					MySQL.Async.fetchAll("SELECT kredit, rata, brplaca FROM users WHERE identifier = @iden", { ['@iden'] = xPlayer.identifier }, function (result6)
						if result6 ~= nil then
							...
							place = place+1
							MySQL.Async.execute("UPDATE users SET brplaca=@brpl WHERE identifier=@identifier", {['@identifier'] = xPlayer.identifier, ['@brpl'] = place})
						end
					end)
				end
			end
