--[[
	-Naredi pregled meta i pretragu njihovu jos i oko baze sve
]]
...
function ProvjeriPosao()
	...
	if PlayerData.job.name == "hitman" then
		ESX.TriggerServerCallback('hitman:DohvatiMete', function(me)
			Mete = me.mete
			Imena = me.imena
		end)
	end
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = _U('sergeant_wear'), value = 'sergeant_wear'},
		{ label = _U('intendent_wear'), value = 'intendent_wear'}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			...
			data.current.value == 'lieutenant_wear'
		then
...
RegisterCommand("contract", function(source, args, rawCommandString)
	local igrac = args[1]
	local cijena = args[2]
	local playerIdx = GetPlayerFromServerId(tonumber(igrac))
	if playerIdx ~= -1 then
		if tonumber(cijena) >= 75000 then
			TriggerServerEvent("hitman:SaljiCifru", igrac, cijena)
		else
			ESX.ShowNotification("Minimalno 75000$!")
			name = "System: "
			message = "/contract [ID][Cijena(Min. 75000$)]"
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    local elements = {
      ...
      {label = 'Uzmi stvari',  value = 'get_stock'},
      {label = 'Ostavi stvari',  value = 'put_stock'}
    }
...
AddEventHandler('esx:onPlayerDeath', function(data)
	...
	if IsEntityAPed(PedKiller) and IsPedAPlayer(PedKiller) then
		Killer = GetPlayerServerId(NetworkGetPlayerIndexFromPed(PedKiller))
	elseif IsEntityAVehicle(PedKiller) and IsEntityAPed(GetPedInVehicleSeat(PedKiller, -1)) and IsPedAPlayer(GetPedInVehicleSeat(PedKiller, -1)) then
		Killer = GetPlayerServerId(NetworkGetPlayerIndexFromPed(GetPedInVehicleSeat(PedKiller, -1)))
	end
	if Killer ~= nil and Killer ~= PlayerId() then
		TriggerServerEvent("hitman:UmroLik", Killer)
	end
end)
...
function OpenFindMenu()
	if Mete ~= nil then
		...
		for _, player in ipairs(GetActivePlayers()) do
			if Mete[GetPlayerName(player)] ~= nil then
				table.insert(mete, {label = Mete[GetPlayerName(player)] .. "$ | " ..Imena[GetPlayerName(player)], value = player})
			end
		end
		...
		{
		  title    = "Izaberite metu koju hocete traziti",
		  align    = 'top-left',
		  elements = mete
		},
		function(data, menu)
			local pos = GetEntityCoords(GetPlayerPed(data.current.value))
			SetNewWaypoint(pos.x, pos.y)
			local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(data.current.value))
			ESX.ShowAdvancedNotification('Meta locirana', Imena[GetPlayerName(data.current.value)], 'Vrijednost mete: '..Mete[GetPlayerName(data.current.value)], mugshotStr, 2)
...
RegisterNetEvent('hitman:UpdateMete')
AddEventHandler('hitman:UpdateMete', function(met, im)
	if PlayerData.job.name == "hitman" then
		Mete = met
		Imena = im
	end
...
RegisterNetEvent('hitman:SaljiMete')
AddEventHandler('hitman:SaljiMete', function(id, cijena, met, ime)
	if PlayerData.job.name == "hitman" then
		Mete = met
		Imena = ime
		local playerIdx = GetPlayerFromServerId(id)
		local ime = GetPlayerName(playerIdx)
		local mugshot, mugshotStr = ESX.Game.GetPedMugshot(GetPlayerPed(playerIdx))
		ESX.ShowAdvancedNotification('Nova meta', Imena[ime], 'Vrijednost mete: '..cijena, mugshotStr, 2)
		UnregisterPedheadshot(mugshot)
	end
end)
...
Citizen.CreateThread(function()
  while true do
	...
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		if IsDisabledControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'hitman' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'hitman_actions') and (GetGameTimer() - GUI.Time) > 150 then
