			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'sipa')

						TriggerServerEvent('esx_service:disableService', 'sipa')
						TriggerEvent('esx_sipa:updateBlip')
		...
		if isInService then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
				...
				elseif data.current.value == 'buy_weapons' then
					OpenBuyWeaponsMenu()
				elseif data.current.value == 'uzmi_dron' then
					TriggerServerEvent("sipa:UzmiDrona")
...
function StoreNearbyVehicle(playerCoords)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		if BVozilo ~= nil then
			...
			ESX.ShowNotification("Nemate izvuceno vozilo!")
		end
	end
end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sipa_actions', {
		title    = 'SIPA',
		align    = 'top-left',
		elements = {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_sipa:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_sipa:handcuff', GetPlayerServerId(closestPlayer), 1)
					elseif action == 'handcuff2' then
						TriggerServerEvent('esx_sipa:handcuff', GetPlayerServerId(closestPlayer), 2)
					elseif action == 'handcuff3' then
						TriggerServerEvent('esx_sipa:handcuff', GetPlayerServerId(closestPlayer), 3)
					elseif action == 'drag' then
						TriggerServerEvent('esx_sipa:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_sipa:putInVehicle', GetPlayerServerId(closestPlayer))
						ClearPedTasks(PlayerPedId())
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_sipa:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_sipa:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('esx_sipa:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('popo:zapljeni69', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
				menu.close()
				Wait(500)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_sipa:getFineList', function(fines)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
			...
			if Config.EnablePlayerManagement then
				TriggerServerEvent('esx_billing:posaljiTuljana', GetPlayerServerId(player), 'society_sipa', _U('fine_total', data.current.fineLabel), data.current.amount)
			else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_sipa:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_sipa:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license',
		{
			...
			TriggerServerEvent('esx_sipa:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_sipa:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_sipa:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('esx_sipa:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('esx_sipa:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_sipa:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_sipa:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_sipa:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_sipa:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_sipa:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_sipa:forceBlip')
end)

RegisterNetEvent('esx_sipa:VratiAnimv2')
AddEventHandler('esx_sipa:VratiAnimv2', function()
...
RegisterNetEvent('esx_sipa:VratiAnim')
AddEventHandler('esx_sipa:VratiAnim', function(tip)
	if tip == 1 then
...
AddEventHandler('esx_sipa:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_sipa:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_sipa:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job and PlayerData.job.name == 'sipa' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_sipa:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_sipa:oslobodiga')
AddEventHandler('esx_sipa:oslobodiga', function()
...
RegisterNetEvent('esx_sipa:handcuff')
AddEventHandler('esx_sipa:handcuff', function(id, tip)
	if tip == 1 then
		IsHandcuffed = true
	elseif tip == 2 then
		IsHandcuffed = true
	...
	Citizen.CreateThread(function()
		if IsHandcuffed then
			...
			if tip == 1 then
			TriggerServerEvent("esx_sipa:SaljiAnim", id, 1)
			ESX.Streaming.RequestAnimDict("mp_arrest_paired", function()
			...
			TriggerServerEvent("esx_sipa:SaljiAnim", id, 2)
			ESX.Streaming.RequestAnimDict("mp_arresting", function()
...
RegisterNetEvent('esx_sipa:unrestrain')
AddEventHandler('esx_sipa:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
		TriggerServerEvent("policija:Cuffan", GetPlayerServerId(PlayerId()), false)
...
RegisterNetEvent('esx_sipa:undragga')
AddEventHandler('esx_sipa:undragga', function()
	dragStatus.isDragged = false
	dragStatus.CopId = nil
end)

RegisterNetEvent('esx_sipa:drag')
AddEventHandler('esx_sipa:drag', function(copId)
	local playerPed = PlayerPedId()
	...
	while true do
		...
		if dragStatus.isDragged then
			if IsPedDeadOrDying(playerPed, true) and animation == false then
				...
				TriggerServerEvent("esx_sipa:SaljiAnimv2", dragStatus.CopId)
...
RegisterNetEvent('esx_sipa:putInVehicle')
AddEventHandler('esx_sipa:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_sipa:OutVehicle')
AddEventHandler('esx_sipa:OutVehicle', function(id)
	local playerPed = PlayerPedId()
	...
	if Mrtav == 1 then 
		...
		dragStatus.CopId = id
		TriggerServerEvent("esx_sipa:SaljiAnimv2", id)
		Wait(200)
		ESX.Streaming.RequestAnimDict("combat@drag_ped@", function()
			TaskPlayAnim(playerPed, "combat@drag_ped@", "injured_drag_ped", 8.0, -8, -1, 33, 0, 0, 0, 0)
		end)
	...
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'sipa' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_sipa:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_sipa:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'sipa' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenu', 'sipa', function(data, menu)
						menu.close()
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'sipa' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'sipa_actions') then
			if Config.MaxInService == -1 then
...
RegisterNetEvent('esx_sipa:updateBlip')
AddEventHandler('esx_sipa:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	...
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_sipa:spawned')
	end
