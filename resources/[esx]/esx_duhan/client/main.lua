...
AddEventHandler('esx_duhan:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_duhan:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()

	if zone == 'Duhan' then
		TriggerServerEvent('esx_duhan:stopHarvestKoda')
	elseif zone == 'ProcessamentoDeVinho' then
	TriggerServerEvent('esx_duhan:stopTransformKoda')
	elseif zone == 'VendaDeBitcon' then
		TriggerServerEvent('esx_duhan:stopSellKoda')
	end
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerServerEvent('esx_duhan:GetUserInventory', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('esx_duhan:hasExitedMarker', lastZone)
		end
		if nasosta == 0 then
			waitara = 500
		end
...
-- itemsy
RegisterNetEvent('esx_duhan:ReturnInventory')
AddEventHandler('esx_duhan:ReturnInventory', function(kodaNbr, kodapNbr, jobName, currentZone)
	kodaQTE			= kodaNbr
	koda_poochQTE	= kodapNbr
	myJob			= jobName
	TriggerEvent('esx_duhan:hasEnteredMarker', currentZone)
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and IsPedOnFoot(PlayerPedId()) then
						...
						if k == "Duhan" then
							...
							--if(GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < Config.ZoneSize.x / 2) then
								...
								if torba == 40 or torba == 41 or torba == 44 or torba == 45 then
									TriggerServerEvent('esx_duhan:startHarvestKoda', true)
								else
									TriggerServerEvent('esx_duhan:startHarvestKoda', false)
								end
							else
								TriggerEvent('esx_duhan:hasExitedMarker', lastZone)
							end
				...
				elseif CurrentAction == 'ProcessamentoDeVinho' then
					TriggerServerEvent('esx_duhan:startTransformKoda')
				elseif CurrentAction == 'VendaDeBitcon' then
					for k,v in pairs(Config.Zones) do
						if k == "VendaDeBitcon" then
							if #(coords-v.Pos) < Config.ZoneSize.x / 2 then
							--if(GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < Config.ZoneSize.x / 2) then
								TriggerServerEvent('esx_duhan:startSellKoda')
							else
								TriggerEvent('esx_duhan:hasExitedMarker', lastZone)
							end
							break
