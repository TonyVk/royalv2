...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITET]', 'Neuspjelo kreiranje lika, pokusajte kasnije ili se javite vlasniku servera!' } })
		end
	end)
end)
...
TriggerEvent('es:addCommand', 'register', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname3 ~= '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITET]', 'Mozete imati max 3 karaktera. Koristite ^3/chardel^0  komandu da obrisete druge karaktere.' } })
		else
			TriggerClientEvent('esx_identity:showRegisterIdentity', source, {})
		end
	end)
end, {help = "Napravite novog karaktera"})

TriggerEvent('es:addGroupCommand', 'char', 'user', function(source, args, user)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Nemate aktivnog karaktera!' } })
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Aktivan karakter: ^2' .. data.firstname .. ' ' .. data.lastname } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Nemate ovlasti!' } })
end, {help = "Daje info tvojeg karaktera"})

TriggerEvent('es:addGroupCommand', 'charlist', 'user', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname1 ~= '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET] Karakter 1:', data.firstname1 .. ' ' .. data.lastname1 } })
			
			if data.firstname2 ~= '' then
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET] Karakter 2:', data.firstname2 .. ' ' .. data.lastname2 } })
				
				if data.firstname3 ~= '' then
					TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET] Karakter 3:', data.firstname3 .. ' ' .. data.lastname3 } })
				end
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITET]', 'Nemate registriranih karaktera. Koristi ^3/register^0 komandu da napravis karaktera.' } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Nemate ovlasti!' } })
...
TriggerEvent('es:addGroupCommand', 'charselect', 'user', function(source, args, user)
	...
	if charNumber == nil or charNumber > 3 or charNumber < 1 then
		TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITET]', 'Krivi karakter!' } })
		return
	end
	...
	getCharacters(source, function(data)
		if charNumber == 1 then
			...
			if data.firstname ~= '' then
				updateIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
					if callback then
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Vas aktivni karakter je ^2' .. data.firstname .. ' ' .. data.lastname } })
					else
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Neuspjesno, javite se adminu!' } })
					end
				end)
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Nemate karaktera na slotu 1!' } })
			end
		elseif charNumber == 2 then
			...
			if data.firstname ~= '' then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Nemate karaktera na slotu 2!' } })
			end
		elseif charNumber == 3 then
			...
			if data.firstname ~= '' then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Nemate karaktera na slotu 3!' } })
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Neuspjesno, javite se adminu!' } })
		end
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Nemate ovlasti!' } })
end, {help = "Switch between character", params = {{name = "char", help = "id karaktera, od 1-3"}}})

TriggerEvent('es:addGroupCommand', 'chardel', 'user', function(source, args, user)
	local charNumber = tonumber(args[1])

	if charNumber == nil or charNumber > 3 or charNumber < 1 then
		TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITET]', 'Nemate tog karaktera!' } })
		return
	end
		...
		if charNumber == 1 then
			...
			if data.firstname ~= '' then
				deleteIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
					if callback then
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Obrisali ste ^1' .. data.firstname .. ' ' .. data.lastname } })
					else
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITET]', 'Neuspjesno, javite se adminu!' } })
					end
				end)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Nemate ovlasti!' } })
end, {help = "Delete a registered character", params = {{name = "char", help = "id karaktera, od 1-3"}}})
