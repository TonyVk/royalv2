...
RegisterNUICallback('register', function(data, cb)
	...
	for theData, value in pairs(myIdentity) do
		if theData == "firstname" or theData == "lastname" then
			number = string.match(value, "%d+")
			if number ~= nil then
				reason = "Ime ili prezime vam ne smije imati brojeve!"
			end
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Vase ime je prekratko ili predugacko (min 2, max 25).'
	end
	...
	if count ~= nameLength then
		return 'Vase ime sadrzi znakove koji nisu dozvoliti.'
	end
	...
	if spacesInName > 2 then
		return 'Vase ime ima 2 razmaka'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'ime vam mora poceti sa velikim slovom.'
	end
