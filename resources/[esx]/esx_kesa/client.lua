...
if closestPlayer == -1 or closestDistance > 2.0 then 
    ESX.ShowNotification('~r~Nema igraca u blizini')
else
  if not HaveBagOnHead then
    TriggerServerEvent('esx_worek:sendclosest', GetPlayerServerId(closestPlayer))
    ESX.ShowNotification('~g~Stavili ste vrecu na glavu od ~w~' .. GetPlayerName(closestPlayer))
    TriggerServerEvent('esx_worek:closest')
  else
    ESX.ShowNotification('~r~Igrac vec ima vrecu na glavi')
  end
end
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~Netko je skinuo vrecu sa vase glave!')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Stavi vrecu na glavu', value = 'puton'},
          {label = 'Skini vrecu sa glave', value = 'putoff'},
