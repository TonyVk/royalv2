	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Pomjerate sa ~y~W,A,S,D~s~ i potvrdjujete sa ~y~SPACE~s~ za lijevi blok kodova.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Pomjerate sa ~y~strelicama~s~ i potvrdjujete sa ~y~ENTER~s~ za desni blok kodova.")
			else
