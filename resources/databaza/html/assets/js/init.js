$(document).ready(function() {
  ...
  $('.back, .logo').click(function() {
    ...
    if ( subPage == null ) {
      ...
      $.post('http://databaza/fetch', JSON.stringify({
        type: 'start'
      }), function( cb ) {
    ...
    if ( offense.length > 0 ) {
      $.post('http://databaza/fetch', JSON.stringify({
        ...
        if ( cb != 'error' ) {
          ...
          $('#search-result p').text('Nema pronadjenih rezultata');
        }
      });
    } else {
      $.post('http://databaza/search', JSON.stringify( {
  ...
  $('#submit-add').click(function() {
    ...
    if ( firstname.length > 0 && lastname.length > 0 && dob.length > 0 && charge.length > 0 && description.length > 0 && term.length > 0 && date.length > 0 ) {
      $.post('http://databaza/add', JSON.stringify({
  ...
  $('#submit-update').click(function() {
    $.post('http://databaza/update', JSON.stringify({
  ...
  $('#submit-delete').click(function() {
    $.post('http://databaza/remove', JSON.stringify({
      offense: $('#signature .r-offense').text()
    }), function( cb ) {
      $('#welcome tbody').html('');
  ...
  $(document).keyup(function(e) {
    if (e.keyCode == 27) {
      reset();
      $.post('http://databaza/escape', JSON.stringify({}));
    }
  });
